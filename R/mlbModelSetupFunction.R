...
mlbModelSetup<-function(modelDate="3_4_2018",pastEvent=FALSE,labModel="WGCBridgestone",postRefresh=NULL,parallelRun=FALSE,cookie=mlbCookies){
...
if(!exists("mlbCookies")){
  ...
  if(sourceData==FALSE){
    ...
    dir.create(paste0("~/MLB_Daily/sourceData/",modelDate,"/",fullSourceData$ContestSuffix[i]),showWarnings = FALSE)
      sourceD <- data.frame(fullSourceData[i,],stringsAsFactors=FALSE)
      write.csv(sourceD,file=paste0("~/MLB_Daily/sourceData/",modelDate,"/",fullSourceData$ContestSuffix[i],"/source.csv"))
      })
  }
  system(command = "cd ~/MLB_Daily/json/ && rm -rf *")
  ...
  sourceDataInForLogs<-read.csv(file=paste0("~/MLB_Daily/sourceData/",modelDate,"/fullSourceData.csv"),stringsAsFactors = FALSE) %>% dplyr::select(StadiumName,SportEventId) %>% unique()
  ...
  write.csv(systemsComplete,file = "~/MLB_Daily/systems.csv")
  ...
  allModel<- mlbModelAll$PlayerModels$Properties %>% dplyr::filter(SourceId==4)
  ...
  if(pastEvent==TRUE){
    ...
    #setup Parallel SNOW cluster with progress bar
      ...
      playerMisc <- foreach::foreach(k=1:iterations, .combine = 'rbind', .options.snow = opts, .packages = c("dplyr"), .errorhandling = "remove") %dopar% {
        ...
        class(mlbModelMisc$Posn)<-"numeric"
        class(mlbModelMisc$TourneyOwnership)<-"numeric"
        class(mlbModelMisc$Rd1)<-"numeric"
        class(mlbModelMisc$Rd2)<-"numeric"
        class(mlbModelMisc$Rd3)<-"numeric"
        class(mlbModelMisc$Rd4)<-"numeric"
      ...
      write.csv(playerMisc,file = paste0("~/MLB_Daily/gameLogs/byDate/",modelDate,".csv"))
    }
    postData<-read.csv(file = paste0("~/MLB_Daily/gameLogs/byDate/",modelDate,".csv")) %>% dplyr::select(-FullName,-X)
  ...
  datePath<-paste0("~/MLB_Daily/dataSets/byDate/",modelDate)
  ...
  nothing2<-foreach::foreach(i=1:nrow(allData)) %do% {
    ...
    coursePath<-paste0("~/MLB_Daily/dataSets/byCourse/",allData[i,]$StadiumName)
    tournamentPath<-paste0("~/MLB_Daily/dataSets/byTournament/",allData[i,]$HomeTeam)
    contestPath<-paste0("~/MLB_Daily/dataSets/byContest/",allData[i,]$ContestSuffix)
